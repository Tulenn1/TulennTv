        ...
        <div style={styles.mediaFolderSection}>
          ...
          {editingFolder ? (
            <div style={styles.folderEditRow}>
              ...
              <button style={{ ...styles.btn, background: 'var(--accent)', color: '#fff' }} onClick={async () => {
                if (folderInput.trim()) {
              ...
              <button style={{ ...styles.btn, background: 'var(--btn-secondary)', color: 'var(--btn-secondary-text)' }} onClick={() => setEditingFolder(false)}>Cancelar</button>
            </div>
          ) : (
          ...
          {!mediaFolder && defaultFolder && (
            <div style={{ marginTop: 16 }}>
              <button
                style={{ ...styles.btn, background: 'var(--accent)', color: '#fff', fontSize: 14, padding: '12px 24px' }}
              ...
              <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>
                Se creará en: <code style={{ background: 'var(--code-bg)', padding: '2px 6px', borderRadius: 4 }}>{defaultFolder}</code>
              </p>
              {initStatus && (
          ...
          {mediaFolder && (
            <button style={{ ...styles.btn, background: 'var(--accent)', color: '#fff', marginTop: 12 }} onClick={handleScanMediaFolder}>
              🔍 Escanear carpeta principal
            </button>
          )}

          <button style={{ ...styles.btn, background: 'var(--btn-secondary)', color: 'var(--btn-secondary-text)', marginTop: 8 }} onClick={async () => {
          ...
          <div style={styles.browserOverlay}>
            <div style={styles.browserModal}>
              <div style={styles.browserHeader}>
                ...
                <button style={{ background: 'none', border: 'none', color: 'var(--text-primary)', fontSize: 18, cursor: 'pointer' }} onClick={() => setShowBrowser(false)}>✕</button>
              </div>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          {loading ? (
            ...
            <div style={styles.empty}>
              <p style={{ fontSize: 18, color: 'var(--text-secondary)' }}>No hay carpetas escaneadas</p>
            </div>
          ) : (
              ...
              {folders.map(f => (
                <div key={f.path} style={styles.card}>
                  ...
                  <div style={styles.cardActions}>
                    ...
                    <button style={{ ...styles.actionBtn, color: 'var(--accent)' }} onClick={() => handleDelete(f.path)}>✕</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={helpStyles.overlay} onClick={() => setShowSettings(false)}>
          <div style={helpStyles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ ...helpStyles.body, gap: 12 }}>
              ...
              <p style={{ ...helpStyles.text, fontSize: 12, color: 'var(--text-muted)' }}>
              ...
              <label style={{ fontSize: 13, color: 'var(--text-secondary)' }}>TMDB API Key:</label>
              <input
...
const styles: Record<string, React.CSSProperties> = {
  ...
  folderPath: { fontSize: 14, color: 'var(--text-secondary)', fontFamily: 'monospace', flex: 1 },
  input: { padding: '8px 12px', background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text-primary)', fontSize: 14 },
  btn: { padding: '8px 16px', background: 'var(--btn-secondary)', color: 'var(--btn-secondary-text)', borderRadius: 6, fontWeight: 600, fontSize: 13, whiteSpace: 'nowrap' },
  helpBtn: {
    width: 28, height: 28, borderRadius: '50%', background: 'var(--btn-secondary)',
    color: 'var(--text-secondary)', border: 'none', fontSize: 14, fontWeight: 700,
  ...
  browserHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', borderBottom: '1px solid var(--border)', fontSize: 14, fontFamily: 'monospace' },
  ...
  selectBtn: { padding: '4px 10px', background: 'var(--accent)', color: '#fff', borderRadius: 4, fontSize: 11, fontWeight: 600, marginLeft: 'auto' },
  ...
  cardCount: { fontSize: 12, color: 'var(--text-muted)' },
  cardActions: { display: 'flex', gap: 8, flexShrink: 0 },
  actionBtn: { padding: '6px 12px', background: 'var(--btn-secondary)', border: 'none', color: 'var(--btn-secondary-text)', borderRadius: 6, fontSize: 14, cursor: 'pointer', fontWeight: 600 },
...
const helpStyles: Record<string, React.CSSProperties> = {
  ...
  modal: { background: 'var(--panel-strong)', borderRadius: 12, width: '100%', maxWidth: 600, maxHeight: '85vh', display: 'flex', flexDirection: 'column', border: '1px solid var(--border)', overflow: 'hidden' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border)' },
  closeBtn: { background: 'transparent', border: 'none', color: 'var(--text-secondary)', fontSize: 20, cursor: 'pointer', padding: 4 },
  body: { flex: 1, overflow: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: 16 },
  text: { fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.7 },
  diagram: { background: 'var(--code-bg)', borderRadius: 8, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 2, fontSize: 14, fontFamily: 'monospace' },
  treeLine: { display: 'flex', alignItems: 'center', gap: 8, padding: '2px 0', flexWrap: 'wrap' },
  treeChild: { paddingLeft: 24, display: 'flex', flexDirection: 'column', gap: 2, borderLeft: '1px solid var(--border)', marginLeft: 8, marginTop: 2, marginBottom: 2 },
  treeGrandchild: { paddingLeft: 24, display: 'flex', flexDirection: 'column', gap: 2, borderLeft: '1px solid var(--border-strong)', marginLeft: 8 },
  folder: { fontSize: 16 },
  video: { fontSize: 14 },
  image: { fontSize: 14 },
  bold: { fontWeight: 700, color: 'var(--text-primary)' },
  tag: { fontSize: 11, color: 'var(--text-muted)', fontStyle: 'italic', fontFamily: 'sans-serif' },
  tips: { display: 'flex', flexDirection: 'column', gap: 8, background: 'var(--code-bg)', borderRadius: 8, padding: 14 },
  tip: { fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 },
  kbd: { background: 'var(--btn-secondary)', color: 'var(--btn-secondary-text)', padding: '2px 6px', borderRadius: 3, fontSize: 12, fontFamily: 'monospace' },
  code: { background: 'var(--bg-primary)', padding: 10, borderRadius: 6, fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5, overflow: 'auto' },
  input: { padding: '10px 14px', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: 6, color: 'var(--text-primary)', fontSize: 14, width: '100%', fontFamily: 'monospace' },
  saveBtn: { padding: '10px 20px', background: 'var(--accent)', color: '#fff', borderRadius: 6, fontWeight: 600, fontSize: 14 },
  fetchBtn: { padding: '10px 20px', background: 'var(--btn-secondary)', color: 'var(--btn-secondary-text)', borderRadius: 6, fontWeight: 600, fontSize: 14 },
}
